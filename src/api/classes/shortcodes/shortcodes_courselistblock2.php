...
class shortcodes_courselistblock2 extends shortcodes {
	/**
	 * @return shortcodes
	 */
	...
	public function planaday_api_courselistblock2( $attributes ) {
				...
				if (
					...
					if ( $course['firstDayPartDate'] !== '' ) {
						if ( ( isset( $this->_options['tooncursusdagdelenverleden'] )
							...
							echo '<div class="pad-course2">';
							echo '<div class="pad-image-block">';
							if ( isset( $this->_options['toonimginoverzicht'] )
							     && $this->_options['toonimginoverzicht'] === '1' ) {
								echo $image;
							}
							...
							echo '</div>';
							echo '<div class="pad-course-block">';
							...
							echo '</div>';
							echo '<div class="pad-details-block">';
							echo '<div class="pad-date2"><i class="fas fa-calendar fa-fw"></i> ' . $startDate;
							...
							if ( isset( $this->_options['startgarantieoverzicht'] )
								...
								echo ' <span id="label_startguaranteed" class="label label-success" title="' . __( "heeft startgarantie",
										"planaday-api" ) . '"><i class="fas fa-check fa-fw"></i> ' . __( "Startgarantie",
										"planaday-api" ) . '</span> ';
							}

							if ( $this->planaday_available_places( $course['usersavailable'], $course['options'] ) <= 0 ) {
								echo '<div class="pad-full"><i class="fas fa-exclamation-triangle fa-fw"></i> ' . __( "Cursus is vol",
										"planaday-api" ) . ' </div> ';
							}
							echo '</div>';
							...
							if ( isset( $this->_options['toondetaildagdelenlijstinoverzicht'] )
								...
								foreach ( $dayparts as $daypart ) {
									...
									if ( isset( $this->_options['toonlabelindagdeel'] )
									     && $this->_options['toonlabelindagdeel']
									     && ! empty( $daypart['labels'] ) ) {
									...
									if ( isset( $this->_options['toonelearningdagdeelinoverzicht'] )
									     && $this->_options['toonelearningdagdeelinoverzicht'] === '1'
									     && $daypart['is_elearning'] === '1' ) {
										echo sprintf( '<li> %s actief vanaf %s %s %s</li>',
											$daypart['name'],
									...
									if ( $daypart['is_elearning'] === '0' ) {
										...
										if ( isset( $this->_options['toonlocatiedagdeeloverzicht'] )
											...
											$location = ' in ' . pad_database::pad_db_part( 'locations', $daypart['locationid'], 'city' );
										}
